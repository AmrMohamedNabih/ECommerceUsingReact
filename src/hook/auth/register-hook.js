...
const RegisterHook = () => {
    ...
    const validationValues = () => {
        ...
        if (password !== confirmPassword) {
            notify("Please confirm your password correctly", "error")
            return;
        }
    }
            ...
            if (res) {
                ...
                if (res.data.errors) {
                    if (res.data.errors[0].msg === "accept only egypt phone numbers")
                        notify("The phone number must be an 11-digit Egyptian number", "error")
                }
                if (res.data.errors) {
                    if (res.data.errors[0].msg === "must be at least 6 chars")
                        notify("The password must be at least 6 characters or numbers", "error")
                }
            }
        }
    }, [loading])
